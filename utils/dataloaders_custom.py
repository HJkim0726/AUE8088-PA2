...
class LoadRGBTImagesAndLabels(LoadImagesAndLabels):
    ...
    def __getitem__(self, index):
        """Fetch R-G-B-T pair with on-the-fly augmentation."""
        ...
        # 1) 이미지 로딩 & Letter-box
        # ───────────────────────────────────────────────────────────
        imgs, (h0s, w0s), hw1s = self.load_image(index)            # 두 모달리티 이미지
        labels            = self.labels[index].copy()        # (N,6) – xywh(좌상) + occ
        shape             = (self.batch_shapes[self.batch[index]]
                             if self.rect else self.img_size)

        # 매번 같은 비율/패딩을 적용하기 위해 첫 모달리티 기준 사용
        img0, ratio, pad  = letterbox(imgs[0], shape,
                                      auto=False, scaleup=self.augment)
        imgs_out = [img0]
        imgs_out.append(letterbox(imgs[1], shape,
                                  auto=False, scaleup=self.augment)[0])

        # (좌상)→(중심) 변환 뒤 xyxy 로
        if labels.size:
            labels[:, 1:3] += labels[:, 3:5] / 2.0            # cx,cy
            h_, w_ = imgs_out[0].shape[:2]
        ...
        # ───────────────────────────────────────────────────────────
        # 2) 공통 기하-변환 (Random-Perspective)
        #    - 동일한 난수 시드를 이용해 두 모달리티에 같은 변환 적용
        # ───────────────────────────────────────────────────────────
        if self.augment:
            rand_state = random.getstate()
            imgs_out[1], labels = random_perspective(
                imgs_out[1], labels,
                degrees=hyp["degrees"],
                translate=hyp["translate"],
                scale=hyp["scale"],
            ...
            imgs_out[0], _ = random_perspective(
                imgs_out[0], labels.copy(),   # thermal 은 label 미사용
                degrees=hyp["degrees"],
                translate=hyp["translate"],
                scale=hyp["scale"],
                shear=hyp["shear"],
        ...
        # ───────────────────────────────────────────────────────────
        # 3) Albumentations → 생략 (필요 시 동일한 방식으로 추가)
        # ───────────────────────────────────────────────────────────
        ...
        # ───────────────────────────────────────────────────────────
        nl = len(labels)
        if self.augment and random.random() < hyp["flipud"]:
            imgs_out = [np.flipud(im) for im in imgs_out]
            if nl:
                labels[:, 2] = 1 - labels[:, 2]

        if self.augment and random.random() < hyp["fliplr"]:
            imgs_out = [np.fliplr(im) for im in imgs_out]
            if nl:
                labels[:, 1] = 1 - labels[:, 1]

        # ───────────────────────────────────────────────────────────
        # 6) xyxy → xywhn (모델 입력 형식)
        # ───────────────────────────────────────────────────────────
        if nl:
            h_out, w_out = imgs_out[0].shape[:2]
        ...
        def _to_tensor(arr):
            """
            (HWC,BGR uint8 or float32) → (CHW,RGB) Torch Tensor
            """
            arr = arr.transpose((2, 0, 1))[::-1]          # CHW, BGR→RGB
            arr = np.ascontiguousarray(arr)               # 메모리 연속화
            return torch.from_numpy(arr)                  # uint8 그대로 두면 후단에서 .float()/255 수행

        imgs_out = [_to_tensor(im) for im in imgs_out]    # 두 모달리티 모두 변환
        ...
        h0_single, w0_single = h0s[1], w0s[1]   # 0: lwir, 1: visible

        # 2) shapes 튜플에 '스칼라'만 넣기
        shapes = ((h0_single, w0_single), (ratio, pad))

        return (imgs_out[0], imgs_out[1]), labels_out, self.im_files[index], \
            shapes, index
